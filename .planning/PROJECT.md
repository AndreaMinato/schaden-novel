...
A 10-novel, 13,318-chapter reading site built with Nuxt 4 + Nuxt UI + Nuxt Content v3. Deployed to Netlify as a server-rendered app. Features clean reading typography, chapter navigation, reading progress tracking, RSS feeds, and sitemaps. Content imported from Google Docs via custom scripts.
...
**Current Milestone: v1.1 SSR Migration**

**Goal:** Switch from full SSG to SSR on Netlify to eliminate 10-min build times.

**Target features:**
- Full SSR deployment on Netlify (server-rendered, no prerendering)
- Nuxt Content v3 serving chapters dynamically at runtime
- Dynamic RSS feeds and sitemaps via server routes
- CDN caching strategy for chapter page performance
- Remove SSG-specific workarounds (body stripping, prerender route generation)
...
**Current state:** v1.0 shipped. 1,096 LOC across TypeScript, Vue, and MJS. 13,318 markdown chapters (170MB). Currently SSG with 26,694 prerendered routes and body-stripped SQL dumps. Migrating to SSR.
...
**Build (v1.0):** `nuxt generate` → `.output/public/` → `netlify deploy --prod`
**Build (v1.1 target):** `nuxt build` → SSR via Netlify Functions
**Import:** `node scripts/import.mjs` (Google Docs → markdown chapters)

## Constraints

- **Stack**: Nuxt 4 + Nuxt UI
- **Hosting**: Netlify (SSR via Netlify Functions)
- **Content**: 13K markdown chapters, SQLite connector (native Node 22.5+)
- **Build**: Target <1 min (SSR, no route prerendering)
...
| SPA fallback for chapters | Prerender all routes, /200.html fallback | ⚠️ Revisit — SSR replaces this |
...
| Post-build SQL dump body stripping | afterParse hook breaks pre-rendering | ⚠️ Revisit — SSR eliminates need |
| Per-novel RSS link-only | rawbody not available in Nuxt Content v3 server queries | ✓ Good |
| Filesystem-based prerender routes | readdirSync reads actual filenames, no hardcoded ranges | ⚠️ Revisit — SSR eliminates prerender |
