  ...
  <action>
    **1. Rewrite `app/pages/index.vue` (home page):**
    ...
    - Template structure:
      ...
      - **Novel sections**: For each novel with chapters, render a heading (novel display name as NuxtLink to `/novels/${slug}`), then a list of chapter entries. Each entry: NuxtLink to `/novels${ch.path}` showing chapter title. Add "View all chapters" link to `/novels/${slug}`.
